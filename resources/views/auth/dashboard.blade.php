...
<body>
    ...
    @foreach ($bildirimler as $bil)
    <div class="alert alert-primary" role="alert">
        {{$bil->content}}
        @if(!$bil->okundu) 
        <form action="{{ route('bildirim.okundu', $bil->id) }}" method="POST"> 
            @csrf 
            <button type="submit">Okundu olarak işaretle</button> </form>
             @else
           <span>(Okundu)</span> 
           @endif
    </div>
    @endforeach
    <div class="row container ml-8">
        <div class="card-deck">
